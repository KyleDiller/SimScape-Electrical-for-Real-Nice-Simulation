...
if NEW_NODE_METHOD==1
    NodeNumber=0;
    for i=1:size(blk,1)

        thisBlock=blk{i,1};
        thisBlock_LConn=blkmask{blk{i,2},2}(1);
        thisBlock_LConnN=blkmask{blk{i,2},3}(1); % indice des LConn Rcomm 'eletrcique;
        if length(blkmask{blk{i,2},2})>1
            thisBlock_RConn=blkmask{blk{i,2},2}(2);
        else
            thisBlock_RConn=0;  % for special case ground
        end
        thisBlock_RConnN=blkmask{blk{i,2},3}(2); % nombre de Lconn et Rconn

        for j=1:thisBlock_LConnN+thisBlock_RConnN
            contyp=blkmask{blk{i,2},5}{j};        
            [a,b]=findConnectedPhysicalBlocks(thisBlock,contyp);

            
            for k=1:size(a,1)
                for m=1:size(blk,1)
                    if isqual(a{k},blk{m,1})
                        for n=1:size(b,1)
                            for p=1:sum(blkmask{blk{i,2},3})
                                if isequal(blkmask{blk{i,2},3})

            if blk(i,j)<0  % means the node has not been assign

              % find if there is one bloc-port that already has a node number

                if blk{i,2}==1 || blk{i,2}==2  % ground case: node =0
                    for k=1:size(a,1)
                        if isqual(a{k},thisBlock)

                    for k=1:size(blk,1)
                        nn=sum(blkmask{blk{i,2},3});% number of node
                    end
                end
            %this is in good progress!
            %[MATE, MATERT]= MakeMate( 'MATE_HDLsolver_inverter_1/MATEREF1', 1);
        end
    end
else
...
%if i>nb_blk
    ...
    [blk, Vnode,cur_br_all, sensorV,sensorI,Vmeas] = MergeCurrentSensorNodes(blk,SLD_block_index);
    ...
    for j=1:size(cur_br_all,1)
        ...
        disp(' ');
        disp('Make netlist ')
        [rlc, source, switches, ~, rlcnames,sourcenames,source_parameter,switchnames,switchtype,switchVf,switch_parameter] = MakeSPSNetlist(blk,blkmask,1000);
        %netlistprint(blk)
        ...
        [yout,y_type,~] = BuildSPSYout(Vnode, switches, cur_br_all,rlcnames,sourcenames,switchnames,Vmeas);
        ...
        [A,B,C,D,~,~,~,~,~,~,~,~,~,~,~,~,~,Outputs]=mate_statespace3(rlc,switches,source,[],yout,y_type);
        ...
        try
        [~, ~]=AddSwitches(modele,switchnames,'InternalSwitchMixer');
        catch
