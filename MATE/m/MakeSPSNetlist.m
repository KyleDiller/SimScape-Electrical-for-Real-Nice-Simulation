...
while i<=size(blk,1)
    ...
    elseif blk{i,2}==3 
        ...
        % end
        rlc=[rlc; blk{i,3} blk{i,4} 0 vals{1}  0     0 ];  % carefull about order node1 node2
    ...
    elseif blk{i,2}==4 
        ...
        rlc=[rlc; blk{i,3} blk{i,4} 0  rval lval*1e3  0 ];    %L in mH 
    ...
    elseif blk{i,2}==5
        ...
        rlc=[rlc; blk{i,3} blk{i,4} 0  rval  0   cval*1e6 ];    %C in uF 
        rlcnames{size(rlc,1),1}=blk{i,1};
        rlcnames{size(rlc,1),2}=i;
        i=i+1;
 ...
 % 28 aout: block is triple in blk{} so we treat the 1st occurance only
  ...
  elseif blk{i,2}==24
    ...
    elseif blk{i,2}==8 %AC Current Source
        ...
        vals=ObtainParameterValue(blk{i,1},paramNames);
        source=[source ; blk{i,4} blk{i,3} 1 vals{1} vals{2} vals{3}];   %blk{i,4} blk{i,3} in this order because current is defined positive from node1 to node 2 internally and it happens that the current source icon has Rconn2 annd LConn1 hcorresponding to node 1 an 2 in this order
      ...
      elseif blk{i,2}==25 %Controlled Current Source
        ...
        % paramNames{3}='frequency';
        % vals=ObtainParameterValue(blk{i,1},paramNames);
        source=[source ; blk{i,4} blk{i,3} 1 0 0 50];   %blk{i,4} blk{i,3} in this order because current is defined positive from node1 to node 2 internally and it happens that the current source icon has Rconn2 and LConn1 hcorresponding to node 1 an 2 in this order
        sourcenames{size(source,1),1}=blk{i,1};
        sourcenames{size(source,1),2}=i;
        len=size(source,1);
        source_parameter{len,1}=blk{i,2};
